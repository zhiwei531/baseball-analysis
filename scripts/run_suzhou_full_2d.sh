...
clips_csv = config_path.parents[1] / "data" / "metadata" / "clips_suzhou_full.csv"
...
for clip_id in "${CLIP_IDS[@]}"; do
  "$PYTHON_BIN" -m baseball_pose.cli --config "$CONFIG_PATH" run-image-proposal-roi --clip-id "$clip_id"
