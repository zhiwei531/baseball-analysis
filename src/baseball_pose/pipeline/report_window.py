...
def detect_action_window(
    ...
    end_index = min(len(parsed) - 1, end_index + pad_after)
    ...
    if current_count > max_frames:
        ...
        end_index = min(len(parsed) - 1, start_index + max_frames - 1)
...
def filter_rows_to_action_window(
    ...
    window = detect_action_window(rows, action_type=action_type)
    if window is None:
        return rows, None
