...
GIF 默认展示关键帧前约 `0.6 s`、后约 `0.4 s` 的窗口，最多 `72` 帧。渲染时使用固定相机视角和固定坐标范围，只画真实身体 marker、`CentreOfMass` 和打击时的 `Bat1-Bat5`。头部 `LFHD/RFHD/LBHD/RBHD` 四点连接为闭合立体面，并全部连接到 `C7`；左右脚的踝、跟、趾三点连接为脚部三角面。
